# سپیدار ۰.۷ — تحویل مرحلهٔ پایداری وظایف و بازیابی
...
IndexedDB: نام `sepidar-offline`، نسخهٔ دیتابیس **۱**، store برابر `app_state`، کلید `current`. ساختار ذخیره:
