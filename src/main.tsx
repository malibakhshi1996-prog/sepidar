...
import './styles.css';
import { addJalaliDays, calendarCells, calendarLabelsForJalali, formatCalendarInput, formatJalaliInput, jalaliAtToDate, jalaliKey, jalaliLabel, jalaliMonthNames, jalaliShortLabel, jalaliViewLabel, jalaliWeekday, jalaliWeekdayNames, moveJalaliCalendar, parseCalendarInput, parseJalaliInput, parseJalaliKey, parseNaturalDateValue, todayJalali, type CalendarSystem } from './core/date';
...
type ZitarCalendarPlugin = { requestPermissions: () => Promise<void>; createEvent: (options: { title: string; description?: string; startAt: number; endAt: number; allDay: boolean }) => Promise<{ eventId: number }> };
...
function App() {
    ...
    const [tabOrder, setTabOrder] = useState<Screen[]>(() => unique([...readStore<Screen[]>('zitar.tabOrder', defaultTabOrder), ...defaultTabOrder]).filter((id) => navItems.some((item) => item.id === id)));
    ...
    useEffect(() => { const handler = () => { const previous = screenHistory.current.pop(); if (previous) { screenRef.current = previous; setScreenState(previous); } else if (Capacitor.isNativePlatform()) { void import('@capacitor/app').then(({ App }) => App.exitApp()).catch(() => undefined); } }; window.addEventListener('zitar-back', handler); return () => window.removeEventListener('zitar-back', handler); }, []);
    useEffect(() => { let remove: (() => void) | undefined; void import('@capacitor/app').then(({ App }) => App.addListener('backButton', () => { const previous = screenHistory.current.pop(); if (previous) { screenRef.current = previous; setScreenState(previous); } else { void App.exitApp(); } }).then(listener => { remove = () => listener.remove(); })).catch(() => undefined); return () => remove?.(); }, []);
    ...
    useEffect(() => { localStorage.setItem('zitar.tabOrder', JSON.stringify(tabOrder)); }, [tabOrder]);
    ...
    const addTask = async (draft: Partial<Task> & Pick<Task, 'title'>) => { if (!draft.title.trim()) return; const created = createTask(draft); setTasks((current) => [created, ...current]); if (created.syncCalendar && created.dateKey && created.time && Capacitor.isNativePlatform()) { const start = jalaliAtToDate(created.dateKey, created.time, created.timezone); if (start) { const end = new Date(start.getTime() + (created.duration || 30) * 60_000); try { await ZitarCalendar.requestPermissions(); const result = await ZitarCalendar.createEvent({ title: created.title, description: created.description || '', startAt: start.getTime(), endAt: end.getTime(), allDay: created.allDay }); setTasks((current) => current.map(task => task.id === created.id ? editTask(task, { calendarEventId: String(result.eventId) }) : task)); } catch { setReminderNotice({ ...created, title: `${created.title} · برای افزودن به تقویم گوشی مجوز لازم است` }); } } } setQuickTitle(''); setShowAdd(false); setTaskView(created.dateKey ? (created.dateKey <= todayKey ? 'today' : 'upcoming') : 'inbox'); setScreen('tasks'); };
    ...
    useEffect(() => { if (!storageReady || !Capacitor.isNativePlatform())
        ...
        const notifications = tasks.filter((task) => isVisibleTask(task) && task.reminder && !task.completed && task.dateKey && task.time).map((task) => { const at = jalaliAtToDate(task.dateKey!, task.time!, task.timezone || 'Asia/Tehran'); if (!at || at.getTime() <= Date.now())
            return null; const id = Math.abs(Array.from(task.id).reduce((hash, char) => ((hash << 5) - hash + char.charCodeAt(0)) | 0, 0)) % 2147480000; return { id: id || 1, title: 'یادآوری زیتر', body: task.title, schedule: { at, allowWhileIdle: true }, channelId: 'task-reminders', smallIcon: 'ic_stat_zitar', extra: { taskId: task.id } }; }).filter(Boolean) as Array<{
            id: number;
            title: string;
            body: string;
            schedule: {
    ...
    useEffect(() => { if (!storageReady || Capacitor.isNativePlatform()) return; const checkReminders = () => { const now = Date.now(); const fired = readStore<string[]>('zitar.reminders.fired', []); const task = tasks.find((item) => isVisibleTask(item) && item.reminder && !item.completed && item.dateKey && item.time && !fired.includes(`${item.id}:${item.dateKey}:${item.time}`) && (() => { const due = jalaliAtToDate(item.dateKey!, item.time!, item.timezone || 'Asia/Tehran'); return due && due.getTime() <= now && now - due.getTime() <= 10 * 60 * 1000; })()); if (!task)
        return; const key = `${task.id}:${task.dateKey}:${task.time}`; const nextFired = [...fired, key].slice(-200); localStorage.setItem('zitar.reminders.fired', JSON.stringify(nextFired)); setReminderNotice(task); if (typeof Notification !== 'undefined' && Notification.permission === 'granted')
        new Notification('یادآوری زیتر', { body: task.title, tag: key }); }; checkReminders(); const timer = window.setInterval(checkReminders, 15000); return () => window.clearInterval(timer); }, [tasks]);
    const updateTask = async (updated: Task) => { if (!updated.title.trim()) return; const currentTask = tasks.find(task => task.id === updated.id); const next = currentTask ? editTask(currentTask, updated) : updated; setTasks((current) => current.map((task) => task.id === updated.id ? next : task)); if (next.syncCalendar && !next.calendarEventId && next.dateKey && next.time && Capacitor.isNativePlatform()) { const start = jalaliAtToDate(next.dateKey, next.time, next.timezone); if (start) { try { await ZitarCalendar.requestPermissions(); const result = await ZitarCalendar.createEvent({ title: next.title, description: next.description || '', startAt: start.getTime(), endAt: start.getTime() + (next.duration || 30) * 60_000, allDay: next.allDay }); setTasks((current) => current.map(task => task.id === next.id ? editTask(task, { calendarEventId: String(result.eventId) }) : task)); } catch { /* The task remains saved even if calendar permission is declined. */ } } } setShowDetail(null); };
    ...
    return <div className={`app-shell ${themeClass}`} style={{ fontSize: `${fontScale}em`, fontFamily: fontFamily === 'Tahoma' ? 'Tahoma, sans-serif' : fontFamily === 'serif' ? 'Georgia, serif' : 'Vazirmatn, Tahoma, sans-serif' }}><main className="app-content">
    ...
    {screen === 'stats' && <><StatsScreen tasks={visibleTasks} focusSessions={focusSessions} habits={habits} onClose={() => setScreen('more')}/><button className="stats-record-fab" onClick={() => setShowFocusRecord(true)} aria-label="افزودن رکورد تمرکز"><Plus size={27}/></button></>}{screen === 'habits' && <HabitsScreen habits={habits} onToggle={toggleHabit} onAdd={() => setShowHabitAdd(true)} onBack={() => setScreen('more')}/>}{screen === 'lists' && <ListsScreen tasks={visibleTasks} lists={unique([...listNames, ...customLists])} onToggle={toggleTask} onEdit={setShowDetail} onAdd={() => setShowListAdd(true)} onBack={() => setScreen('more')}/>}{screen === 'settings' && <SettingsScreen tab={settingsTab} setTab={setSettingsTab} activeTheme={activeTheme} setActiveTheme={setActiveTheme} tabOrder={tabOrder} setTabOrder={setTabOrder} tabLimit={tabLimit} setTabLimit={setTabLimit} fontScale={fontScale} setFontScale={setFontScale} fontFamily={fontFamily} setFontFamily={setFontFamily} onBack={() => setScreen('more')}/>}
...
function CalendarScreen({ tasks, onToggle, onEdit, onAdd }: {
    ...
    const [enabledCategories, setEnabledCategories] = useState<EventCategory[]>(() => { const stored = readStore<EventCategory[] | null>('zitar.eventCategories', null); return stored ? unique(stored) : eventCategories; });
    ...
    const toggleEventCategory = (category: EventCategory) => setEnabledCategories((current) => { const next = current.includes(category) ? current.filter((item) => item !== category) : [...current, category]; localStorage.setItem('zitar.eventCategories', JSON.stringify(next)); return next; });
...
function FocusSettings({ presetMinutes, onPresetChange, onClose, onOpenRecord }: { presetMinutes: number; onPresetChange: (minutes: number) => void; onClose: () => void; onOpenRecord: () => void }) {
    const [flipStart, setFlipStart] = useState(() => readStore('zitar.focus.flipStart', false));
    const [strictMode, setStrictMode] = useState(() => readStore('zitar.focus.strictMode', false));
    const [antiBurnIn, setAntiBurnIn] = useState(() => readStore('zitar.focus.antiBurnIn', true));
    const [sync, setSync] = useState(() => readStore('zitar.focus.sync', false));
    const [fullScreen, setFullScreen] = useState(() => readStore('zitar.focus.fullScreen', false));
    const [showPomo, setShowPomo] = useState(false);
    const update = (key: string, value: boolean, setter: (value: boolean) => void) => { setter(value); localStorage.setItem(`zitar.focus.${key}`, JSON.stringify(value)); };
...
function SettingsScreen({ tab, setTab, activeTheme, setActiveTheme, tabOrder, setTabOrder, tabLimit, setTabLimit, fontScale, setFontScale, fontFamily, setFontFamily, onBack }: {
    tab: SettingsTab;
    ...
    onBack: () => void;
}) { return <section className="screen settings-screen"><div className="settings-tabs"><button className={tab === 'display' ? 'active' : ''} onClick={() => setTab('display')}>نمایش</button><button className={tab === 'icons' ? 'active' : ''} onClick={() => setTab('icons')}>آیکون برنامه</button><button className={tab === 'theme' ? 'active' : ''} onClick={() => setTab('theme')}>پوسته</button><button className={tab === 'tabbar' ? 'active' : ''} onClick={() => setTab('tabbar')}>تب‌ها</button><button className={tab === 'dateTime' ? 'active' : ''} onClick={() => setTab('dateTime')}>تاریخ و زمان</button><button className="icon-button back" onClick={onBack} aria-label="بازگشت"><ArrowLeft size={29}/></button></div>{tab === 'display' && <DisplaySettings fontScale={fontScale} setFontScale={setFontScale} fontFamily={fontFamily} setFontFamily={setFontFamily} onDateTime={() => setTab('dateTime')}/>} {tab === 'dateTime' && <DateTimeSettings onBack={() => setTab('display')}/>} {tab === 'icons' && <IconSettings />}{tab === 'theme' && <ThemeSettings activeTheme={activeTheme} setActiveTheme={setActiveTheme}/>} {tab === 'tabbar' && <TabBarSettings order={tabOrder} setOrder={setTabOrder} limit={tabLimit} setLimit={setTabLimit}/>}</section>; }
function DisplaySettings({ fontScale, setFontScale, fontFamily, setFontFamily, onDateTime }: { fontScale: number; setFontScale: (scale: number) => void; fontFamily: string; setFontFamily: (family: string) => void; onDateTime: () => void }) { const [sidebarCount, setSidebarCount] = useState<'hide' | 'show'>('show'); const [listColor, setListColor] = useState<'hide' | 'show'>('show'); return <div className="settings-body"><SettingsCard title="نوع فونت"><div className="frequency-options"><button className={fontFamily === 'Vazirmatn' ? 'active' : ''} onClick={() => setFontFamily('Vazirmatn')}>وزیرمتن</button><button className={fontFamily === 'Tahoma' ? 'active' : ''} onClick={() => setFontFamily('Tahoma')}>Tahoma</button><button className={fontFamily === 'serif' ? 'active' : ''} onClick={() => setFontFamily('serif')}>سریف</button></div></SettingsCard><SettingsCard title="اندازه فونت"><div className="frequency-options">{[[0.9, 'کوچک'], [1, 'پیش‌فرض'], [1.15, 'بزرگ'], [1.3, 'خیلی بزرگ']].map(([value, label]) => <button key={String(value)} className={fontScale === value ? 'active' : ''} onClick={() => setFontScale(value as number)}>{label}</button>)}</div></SettingsCard><SettingsRow title="تاریخ و زمان" value="جلالی · شنبه" onClick={onDateTime}/><SettingsRow title="ویجت‌ها" value="ویجت امروز"/><SettingsCard title="تعداد میانبرها"><div className="choice-row"><ChoiceCard title="مخفی" selected={sidebarCount === 'hide'} onClick={() => setSidebarCount('hide')} icon={<List size={34}/>}/><ChoiceCard title="نمایش" selected={sidebarCount === 'show'} onClick={() => setSidebarCount('show')} icon={<List size={34}/>}/></div><ToggleRow title="یادداشت‌ها را مخفی کن"/></SettingsCard><SettingsCard title="رنگ فهرست"><div className="choice-row"><ChoiceCard title="مخفی" selected={listColor === 'hide'} onClick={() => setListColor('hide')}/><ChoiceCard title="نمایش" selected={listColor === 'show'} onClick={() => setListColor('show')} accent/></div><p className="helper">رنگ فهرست‌ها در لیست کارها نمایش داده شود.</p></SettingsCard><SettingsCard title="نمایش کار تکمیل‌شده"><ToggleRow title="خط‌زدن عنوان کار"/></SettingsCard></div>; }
function DateTimeSettings({ onBack }: { onBack: () => void }) { const [timeFormat, setTimeFormat] = useState<'24' | '12'>(() => readStore<'24' | '12'>('zitar.timeFormat', '24')); const [startDay, setStartDay] = useState<'saturday' | 'sunday'>(() => readStore<'saturday' | 'sunday'>('zitar.startDay', 'saturday')); const [calendarLabels, setCalendarLabels] = useState(() => readStore('zitar.calendarLabels', { jalali: true, gregorian: true, hijri: true })); const [weekNumbers, setWeekNumbers] = useState(() => readStore('zitar.weekNumbers', false)); const [timezone, setTimezone] = useState(() => readStore('zitar.timezone', false)); const save = <T,>(key: string, value: T, setter: (value: T) => void) => { setter(value); localStorage.setItem(`zitar.${key}`, JSON.stringify(value)); }; return <div className="settings-body"><div className="inner-header"><button className="icon-button" onClick={onBack} aria-label="بازگشت"><ArrowRight size={25}/></button><h1>تاریخ و زمان</h1></div><SettingsCard title="فرمت زمان"><div className="choice-row"><ChoiceCard title="۲۴ ساعته" selected={timeFormat === '24'} onClick={() => save('timeFormat', '24', setTimeFormat)} icon={<Clock size={30}/>}/><ChoiceCard title="۱۲ ساعته" selected={timeFormat === '12'} onClick={() => save('timeFormat', '12', setTimeFormat)} icon={<Clock size={30}/>}/></div></SettingsCard><SettingsCard title="شروع هفته"><div className="frequency-options"><button className={startDay === 'saturday' ? 'active' : ''} onClick={() => save('startDay', 'saturday', setStartDay)}>شنبه</button><button className={startDay === 'sunday' ? 'active' : ''} onClick={() => save('startDay', 'sunday', setStartDay)}>یکشنبه</button></div></SettingsCard><SettingsCard title="تقویم‌های قابل نمایش"><PreferenceToggle title="تقویم شمسی" description="تقویم اصلی و مناسبت‌های ایران." value={calendarLabels.jalali} onChange={(value) => save('calendarLabels', { ...calendarLabels, jalali: value }, setCalendarLabels)}/><PreferenceToggle title="تقویم میلادی" description="تاریخ میلادی کنار تاریخ شمسی نمایش داده شود." value={calendarLabels.gregorian} onChange={(value) => save('calendarLabels', { ...calendarLabels, gregorian: value }, setCalendarLabels)}/><PreferenceToggle title="تقویم قمری" description="تاریخ قمری دستگاه در کنار دو تقویم دیگر نمایش داده شود." value={calendarLabels.hijri} onChange={(value) => save('calendarLabels', { ...calendarLabels, hijri: value }, setCalendarLabels)}/></SettingsCard><SettingsCard title="گزینه‌های تقویم"><PreferenceToggle title="نمایش شماره هفته" description="شمارهٔ هفته در نماهای تقویم نمایش داده شود." value={weekNumbers} onChange={(value) => save('weekNumbers', value, setWeekNumbers)}/><PreferenceToggle title="انتخاب منطقهٔ زمانی" description="در زمان‌بندی کارها منطقهٔ زمانی قابل انتخاب باشد." value={timezone} onChange={(value) => save('timezone', value, setTimezone)}/></SettingsCard></div>; }
function IconSettings() { const [selected, setSelected] = useState(0); const icons = [<FlowerLotus size={42} weight="duotone"/>, <Check size={42} weight="bold"/>, <Target size={42}/>]; return <div className="settings-body"><SettingsCard title="آیکون برنامه"><div className="icon-options">{icons.map((item, index) => <button key={index} className={`app-icon ${selected === index ? 'selected' : ''}`} onClick={() => setSelected(index)} aria-label={`آیکون ${index + 1}`}>{item}</button>)}</div><p className="helper">آیکون زیتر را برای صفحه اصلی انتخاب کن.</p></SettingsCard></div>; }
...
function CreateTaskSheet({ initialTitle, lists, onClose, onSave }: { initialTitle: string; lists: string[]; onClose: () => void; onSave: (draft: Partial<Task> & Pick<Task, 'title'>) => void }) {
  ...
  const valid = Boolean(title.trim() && !dateError && !timeError && (!time || selectedDate))
  ...
  const save = () => { if (!valid) return; onSave({ title, description: description || undefined, list, dateSystem, dateKey: selectedDate ? jalaliKey(selectedDate) : undefined, date: selectedDate ? jalaliShortLabel(selectedDate) : undefined, time: time || undefined, duration, reminder: reminder && Boolean(selectedDate && time), syncCalendar: syncCalendar && Boolean(selectedDate && time), timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'Asia/Tehran', priority, matrixQuadrant, recurrence: recurrence || undefined }) }
  ...
  return <div className="sheet-backdrop" onClick={onClose}><section className="quick-sheet create-task-sheet" onClick={(event) => event.stopPropagation()}><div className="sheet-handle"/><div className="sheet-title"><strong>افزودن کار کامل</strong><button className="icon-button" onClick={onClose} aria-label="بستن"><X size={24}/></button></div>
    ...
    <label className="field-label">مهلت در تقویم {dateSystem === 'JALALI' ? 'شمسی' : dateSystem === 'GREGORIAN' ? 'میلادی' : 'قمری'}<input className="habit-input" dir="ltr" value={dateText} onChange={event => setDateText(event.target.value)} placeholder={dateSystem === 'JALALI' ? '۱۴۰۵/۰۶/۲۷' : dateSystem === 'GREGORIAN' ? '۲۰۲۶/۰۹/۱۸' : '۱۴۴۷/۰۳/۲۶'}/></label>
    ...
    {timeError && <p className="helper" role="alert">ساعت معتبر نیست.</p>}<label className="editor-check"><input type="checkbox" checked={reminder && Boolean(selectedDate && time)} disabled={!selectedDate || !time} onChange={event => setReminder(event.target.checked)}/> یادآوری با زنگ دستگاه</label><label className="editor-check"><input type="checkbox" checked={syncCalendar && Boolean(selectedDate && time)} disabled={!selectedDate || !time} onChange={event => setSyncCalendar(event.target.checked)}/> ثبت این قرار در تقویم گوشی</label>
    <div className="editor-fields"><label className="field-label">اولویت<select className="habit-input" value={priority} onChange={event => { const value = event.target.value as Priority; setPriority(value); setMatrixQuadrant(matrixQuadrantForPriority(value)) }}><option value="none">بدون اولویت</option><option value="low">کم</option><option value="medium">متوسط</option><option value="high">زیاد</option></select></label><label className="field-label">فهرست<select className="habit-input" value={list} onChange={event => setList(event.target.value)}>{lists.map(item => <option key={item} value={item}>{item}</option>)}</select></label></div>
