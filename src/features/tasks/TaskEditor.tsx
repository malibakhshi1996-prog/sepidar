...
import { addJalaliDays, calendarCellsForSystem, calendarViewLabelForSystem, formatCalendarInput, jalaliKey, jalaliLabel, jalaliShortLabel, jalaliWeekdayNames, moveCalendarForSystem, parseCalendarInput, parseJalaliKey, todayJalali, type CalendarSystem, type WeekStart } from '../../core/date'
...
export function CalendarPicker({ value, system = 'JALALI', weekStart = 'saturday', onChange }: { value?: string; system?: CalendarSystem; weekStart?: WeekStart; onChange: (key: string) => void }) {
  const [month, setMonth] = useState(() => ({ ...(parseJalaliKey(value || '') || todayJalali()), day: 1 }))
  const days = calendarCellsForSystem(month, 'month', system, weekStart)
  const selectedKey = value || ''
  return <div className="inline-calendar">
    <div className="calendar-month"><button type="button" className="icon-button" aria-label="ماه قبل" onClick={() => setMonth(moveCalendarForSystem(month, 'month', -1, system))}><ArrowRight size={20}/></button><strong>{calendarViewLabelForSystem(month, 'month', system, weekStart)}</strong><button type="button" className="icon-button" aria-label="ماه بعد" onClick={() => setMonth(moveCalendarForSystem(month, 'month', 1, system))}><ArrowLeft size={20}/></button></div>
    <div className="month-weekdays">{['شنبه', 'یکشنبه', 'دوشنبه', 'سه‌شنبه', 'چهارشنبه', 'پنجشنبه', 'جمعه'].map(day => <span key={day}>{day.slice(0, 2)}</span>)}</div>
    <div className="month-grid">{days.map(day => <button type="button" key={day.key} aria-label={`${jalaliLabel(day)} · ${formatCalendarInput(day, system)}`} aria-pressed={day.key === selectedKey} className={`${day.key === selectedKey ? 'selected-month-day' : ''} ${day.inRange ? '' : 'outside'} ${day.weekday === 6 ? 'holiday' : ''}`} onClick={() => onChange(day.key)}>{fa(day.displayDay)}</button>)}</div>
...
export function TaskEditor({ task, lists, onClose, onSave, onDelete }: {
  ...
  const save = () => { if (!valid) return; onSave({ ...draft, dateKey: normalizedDate, dateSystem, date: selectedDate ? jalaliShortLabel(selectedDate) : '', time, reminder: draft.reminder && !!selectedDate && !!time, syncCalendar: syncCalendar && !!selectedDate }) }
  ...
  return <dialog ref={dialog} className="detail-sheet task-dialog" aria-labelledby="editor-title" onCancel={onClose}>
    <form onSubmit={event => { event.preventDefault(); save() }}>
      ...
      <div className="editor-fields"><label className="editor-field">تاریخ {dateSystem === 'JALALI' ? 'شمسی' : dateSystem === 'GREGORIAN' ? 'میلادی' : 'قمری'}<input dir="ltr" placeholder={dateSystem === 'JALALI' ? '۱۴۰۵/۰۶/۲۷' : dateSystem === 'GREGORIAN' ? '۲۰۲۶/۰۹/۱۸' : '۱۴۴۷/۰۳/۲۶'} aria-invalid={dateError} value={dateText} onChange={e => setDateText(e.target.value)}/></label><button type="button" className="detail-button" onClick={() => setPicker(!picker)} aria-expanded={picker}><CalendarBlank size={20}/> انتخاب تاریخ</button></div>
      {picker && <CalendarPicker system={dateSystem} value={normalizedDate} onChange={key => { setDateText(formatCalendarInput(parseJalaliKey(key) || todayJalali(), dateSystem)); setPicker(false) }}/>} {dateError && <p role="alert">تاریخ {dateSystem === 'JALALI' ? 'جلالی' : dateSystem === 'GREGORIAN' ? 'میلادی' : 'قمری'} معتبر نیست.</p>}
      ...
      <label className="editor-check"><input type="checkbox" checked={syncCalendar && !!selectedDate} disabled={!selectedDate} onChange={e => setSyncCalendar(e.target.checked)}/> ثبت در تقویم گوشی {time ? '' : '(تمام‌روز)'}</label>
